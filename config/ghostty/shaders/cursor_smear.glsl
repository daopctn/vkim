...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Blend trail color with background (with trail transparency)
    const float trailOpacity = 0.5;
    vec4 originalColor = fragColor;
    fragColor.rgb = mix(fragColor.rgb, enhancedColor.rgb, alpha * trailOpacity);
